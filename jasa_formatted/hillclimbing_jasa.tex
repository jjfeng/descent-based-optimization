...
\end{equation}
As before, we fit the model using least squares, and add penalties to induce preferred behaviors. In particular, we are interested in smooth estimates $\hat{\boldsymbol{\theta}}^{(i)} \in \mathbb{R}^n$ for the function $f_i$ at points $x_{i1}, ..., x_{in}$ using Hodrick-Prescott (H-P) filtering. Furthermore, we are interested in sparse estimates $\hat{\boldsymbol{\beta}}$, so we also add a lasso penalty.

To formalize our optimization problem we give a bit of notation. Let $\boldsymbol{Z}_T \in \mathbb{R}^{|T| \times q}, \boldsymbol{Z}_V \in \mathbb{R}^{|V| \times q}$ be the linear covariates from the training and validation sets. Let $\boldsymbol X \in \mathbb{R}^{n \times p}$ be the design matrix for the nonlinear covariates where the first $|T|$ observations are from the training set and the last $|V|$ observations are from the validation set. Let $\boldsymbol{I}_T$ as the first $|T|$ rows and $\boldsymbol{I}_V$ as the last $|V|$ rows of an $n\times n$ identity matrix. We combine the validation and training nonlinear covariates into one design matrix since we use the training data to fit estimates for $f_i$ at the validation points. 
