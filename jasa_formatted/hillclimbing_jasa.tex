...
\subsection{Additive model with heterogeneous smoothness}\label{sec:additive}
...
It is easy to check that all three conditions for Theorem~\ref{thethrm} are satisfied. The problem is composed solely of quadratic functions, so the validation loss $L(\boldsymbol {y}_v, \boldsymbol{I}_V \sum_{i=1}^p \hat{\boldsymbol{\theta}}^{(i)}(\boldsymbol{\lambda}))$ differentiable everywhere with respect to $\boldsymbol{\lambda}$. Choosing $\epsilon >0$ will satisfy the positive definite condition.
...
\begin{equation}
\nabla_{\boldsymbol \lambda} L(\boldsymbol y_V, \boldsymbol{I}_V \sum_{i=1}^p \hat{\boldsymbol{\theta}}^{(i)}(\boldsymbol{\lambda})) =
- \left (
...
\includegraphics[width=1.1\textwidth] {threegam/train_gs_v_hc_f3_0.pdf}
\caption{$f_1$}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=1.1\textwidth]  {threegam/train_gs_v_hc_f3_1.pdf}
\caption{$f_2$}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=1.1\textwidth]  {threegam/train_gs_v_hc_f3_2.pdf}
\caption{$f_3$}
\end{subfigure}
