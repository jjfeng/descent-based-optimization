...
We used the same simulation settings as those in Section \ref{sec:simulationAPLM}. Gradient descent was initialized at $\boldsymbol{\lambda} = 10^i \times \boldsymbol 1_3^\top$ for $i=-4, ..., 1$. We experimented with three nonlinear functions $g:\mathbb{R} \mapsto \mathbb{R}$ of varying levels of smoothness, as given in Table \ref{ref:aplm}.
...
\section{NEW: Trend Filtering}\label{sec:trendfilter}

Nonparametric regression via trend filtering. ....paragraph goes here....

We suppose observations $y$ come from the model
\begin{equation}
y = \sum_{m=1}^M f_m(x_m) + \epsilon
\end{equation}
where $x_m$ are the observed covariates, $f_m$ are the $M$ functions to be estimated, and $\epsilon$ are independent errors. If we believe that the functions are smooth, the functions can be estimated using the 2nd order $\ell_2$ trend filtering estimate defined as
\begin{equation}
\hat{\theta} = \argmin_{\theta^{(m)} \in \mathbb{R}^n} \frac{1}{2n} \| y - \sum_{m=1}^M \theta^{(m)} \|^2_2 + \lambda \sum_{m=1}^M \| D^{(2)}_m \theta^{(m)} \|_2^2
\end{equation}
...
\end{equation}
Of course the difficulty is in choosing the penalty parameters. Ideally, $\lambda_m$ is large for $f_m$ with nearly constant first-order derivatives and small for $f_m$ with drastically changing first-order derivatives. Through this example, we find that descent-based joint optimization tunes the penalty parameters appropriately.

The joint optimization problem is formulated as
\begin{equation}
\begin{array}{c}
\min_{\boldsymbol{\lambda} \in \mathbb{R}^M_+} \frac{1}{2} \left \| y_V - (\boldsymbol{I} - \boldsymbol{I}_T) \sum_{m=1}^M \hat{\boldsymbol{\theta}}^{(m)}(\boldsymbol{\lambda}) \right \|^2_2 \\
\hat{\boldsymbol{\theta}}^{(m)}(\boldsymbol{\lambda}) = 
\min_{\boldsymbol{\theta}^{(m)} \in \mathbb{R}^n} \frac{1}{2} \left \| \boldsymbol{y}_T
- \boldsymbol{I}_T \sum_{m=1}^M \boldsymbol{\theta}^{(m)} \right \|^2_2
+ \sum_{m=1}^M \lambda_m \left \| D^{(2)}_m \boldsymbol{\theta}^{(m)} \right \|_2^2
+ \epsilon \| \boldsymbol{\theta} \|_2^2
\end{array}
\end{equation}

Thru gradient crunching, you get
\begin{equation}
\boldsymbol{W}
\left (
\begin{array}{c}
...
... \\
\frac{\partial}{\partial \boldsymbol \lambda} \hat{\boldsymbol{\theta}}^{(M)}(\boldsymbol \lambda)\\
\end{array}
\right )
 =
\left (
\begin{array}{c}
\boldsymbol{0}\\
\boldsymbol{D}^{(2)\perp}_m \boldsymbol{D}^{(2)}_m \hat{\boldsymbol{\theta}}^{(m)}(\boldsymbol \lambda) \\
\boldsymbol{0}
\end{array}
\right )
\end{equation}
...
\left (
\begin{array}{cccc}
\boldsymbol{I}_T^\perp \boldsymbol{I}_T + \lambda_1 \boldsymbol{D}^{(2)\perp}_1 \boldsymbol{D}^{(2)}_1 + \epsilon \boldsymbol{I} &
\boldsymbol{I}_T^\perp \boldsymbol{I}_T &
... &
\boldsymbol{I}_T^\perp \boldsymbol{I}_T\\
\boldsymbol{I}_T^\perp \boldsymbol{I}_T &
\boldsymbol{I}_T^\perp \boldsymbol{I}_T + \lambda_1 \boldsymbol{D}^{(2)\perp}_2 \boldsymbol{D}^{(2)}_2 + \epsilon \boldsymbol{I} &
... &
\boldsymbol{I}_T^\perp \boldsymbol{I}_T\\
... & ... & ... & ... \\
\boldsymbol{I}_T^\perp \boldsymbol{I}_T &
\boldsymbol{I}_T^\perp \boldsymbol{I}_T &
... &
\boldsymbol{I}_T^\perp \boldsymbol{I}_T + \lambda_1 \boldsymbol{D}^{(2)\perp}_M \boldsymbol{D}^{(2)}_M + \epsilon \boldsymbol{I}\\
\end{array}
\right )
\end{equation}
