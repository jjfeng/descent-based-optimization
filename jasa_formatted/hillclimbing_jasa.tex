...
\end{equation}
\text{s.t. } $\{P_i\}_{i=1, ..., J}$ are the penalty functions, and $\boldsymbol{\lambda} = (\lambda_1, \ldots, \lambda_J)^\top$ are the regularization parameters.
...
\label{jointopt2}
\end{equation}
For the remainder of the manuscript we will assume that \eqref{eq:train_disc} is strictly convex in $\boldsymbol \theta$. This ensures that there is a unique solution to \eqref{eq:train_disc} which perturbs continuously in $\boldsymbol{\lambda}$.
...
\hline
Generalized Lasso & $\mathcal{N}(I_{I(\boldsymbol \lambda)} \boldsymbol D)$ where $I(\boldsymbol \lambda) = \{ i | \left(\boldsymbol D \hat{\boldsymbol{\theta}}(\boldsymbol \lambda)\right)_i = 0  \text{ for } i=1,...,p \}$ \\
\hline
...
Let $I(\lambda)$ denote the indices of the zero elements of $\boldsymbol D \hat{\boldsymbol{\theta}}(\lambda)$, ie $I(\lambda) = \left \{i | (\boldsymbol D \hat{\boldsymbol{\theta}}(\lambda))_i = 0  \text{ for } i=1,...,p \right \}$.
...
In this example we assume that on each of $i=1,\ldots n$ observations we have measured a response $y_i$, a vector of ``linearly modeled features'' $\boldsymbol{x}_i$, and a single ``continuously modeled feature'' $z_i$. We believe that $\boldsymbol y$ can be modeled as an additive combination of these features:
\begin{equation}
...
In this example, the lasso is the part which is not everywhere differentiable. Let the nonzero indices of $\hat{\boldsymbol{\beta}}(\boldsymbol\lambda)$ be denoted $I(\boldsymbol\lambda) = \{i | \hat{\beta}_i(\boldsymbol\lambda) \ne 0 \text{ for } i=1,...,p \}$. The differentiable space is then
\begin{equation}
...
\end{equation}
and $\boldsymbol \epsilon \sim N(0, \boldsymbol I)$. $\sigma$ was chosen such that the signal to noise ratio is 2. 
...
\end{equation}
where $\boldsymbol \beta^{(j)} = (1, 2, 3, 4, 5, 0, ..., 0)$ for $j = 1, 2, 3$ and $\boldsymbol \epsilon \sim N(0, \boldsymbol I)$. $\sigma$ was chosen such that the signal to noise ratio was 2. For the sparse group lasso, we used $M=150$ covariate groups with $10$ covariates each.
...
\end{array}
\end{equation}
where $Z_1 \sim N(0,1)$, $Z_2 \sim N(0,1)$, and $\delta_i \sim N(0, \frac{1}{16})$. Nonlinear predictors $\boldsymbol z$ were randomly drawn from the standard uniform distribution. The response $\boldsymbol y$ was generated from the model
\begin{equation}
...
+ \lambda_1 \| \boldsymbol \beta \|_1
+ \frac{1}{2} \lambda_2 \| \boldsymbol \beta \|_2^2
+ \frac{1}{2} \lambda_3 \| \boldsymbol D^{(2)} \boldsymbol \theta \|_2^2
\end{equation}
...
In addition to judging models by their performances on the test set, we measured the error of the fitted linear effects and the nonparametric estimates. These correspond to the $\beta$ error ($||\boldsymbol \beta - \hat {\boldsymbol \beta}||_2^2$) and $\theta$ error ($|| g(\boldsymbol z) - \boldsymbol \theta ||_2^2$), respectively.
...
\bibliography{hillclimbing_jasa}
