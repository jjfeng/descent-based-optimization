...
%		
%		In addition, it should be noted that, while the current-gold-standard solvers for Lasso problems (with a fixed $\lambda$) do leverage sparsity, sub-gradient descent with a properly chosen step-size will converge to the solution to the lasso problem. In addition, if initialized at a differentiable $\beta$-value (ie. with all non-zero entries); then with probability $1$, the lasso objective will be differentiable at all iterates in this procedure. These sub-gradient methods have fallen out of favor as none of the iterates will be sparse (though they limit to the true sparse solution) and the convergence rate after $k$ steps is $\frac{1}{\sqrt{k}}$. In contrast, generalized gradient/accelerated generalized gradient methods give sparse solutions at large enough iterates and achieve rates of $\frac{1}{k}$ and $\frac{1}{k^2}$ \citet{nesterov2013introductory}.
