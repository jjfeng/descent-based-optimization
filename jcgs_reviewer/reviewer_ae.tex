...
\begin{document}
	...
	\begin{enumerate}
		...
		We solved a penalized least squares problem with a lasso penalty and tuned the penalty parameter using a validation set. The penalty parameter that minimized the validation loss was never located at a knot. More specifically, using a homotopy solver for the lasso, we were able to find the \emph{exact} knots, and these points never achieved the minimum value. While this is not definitive proof, we believe it is a strong indication that it is unlikely for solutions to occur regularly at knots in penalized problems.
