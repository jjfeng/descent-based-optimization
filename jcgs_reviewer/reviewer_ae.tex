...
\begin{document}
	...
	\begin{enumerate}
		...
		\reply 	
		We agree that the condition of interest is whether the loss is smooth almost everywhere with respect to the probability measure induced by the true sampling model. We suspect that the set of knots, the penalty parameters at which the validation loss is not differentiable, has measure zero under the true sampling model. Gradient descent is agnostic to whether or not the training criterion is smooth, so it seems unlikely that it will prefer knots. \textcolor{red}{This paragraph (And maybe the whole response) needs work --- for example subgradient descent for the lasso is also agnostic, but that doesn't imply that the solution doesn't occur at knots. In fact, it actually implies that subgradient descent is a bad method \emph{because} solutions occur at knots!}
		
		We also empirically investigated whether the minimizer of the validation loss tended to be at knots. We performed a simulation study with a penalized least squares problem with a lasso penalty.  Over \textcolor{red}{BLAH simulations} The penalty parameter that minimized the validation loss was never located at a knot. More specifically, using a homotopy solver for the lasso, we were able to find the \emph{exact} knots, and these points never achieved the minimum value. While this is not definitive proof, we believe it is a strong indication that it is unlikely for solutions to occur regularly at knots in penalized problems.
		
		\textbf{Simulation settings} We considered a linear model with 50 covariates. The training and validation sets included 15 and 10 observations, respectively. \textcolor{red}{Can we do this with like 30+ observations? 15 and 10 are tiny numbers.} The response was generated data from the model
		$$
