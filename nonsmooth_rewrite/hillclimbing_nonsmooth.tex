...
The Hessian $\boldsymbol{H}(\boldsymbol\lambda)$ and matrix $\boldsymbol{C}(\hat{\boldsymbol \beta}( \boldsymbol \lambda))$ are defined in the Appendix.
...
\end{equation}
with $\epsilon$ iid errors. Of this matrix $\boldsymbol{M}$, we only observe some subset of the positions. Let the observed matrix positions be partitioned into a training set $T$, and a validation set $V$. Let $\| \cdot \|^2_T$ (resp. $\| \cdot \|^2_V$) denote the Frobenius norm over the entries in $T$ (resp. $V$). For simplicity, we suppose the number of row and column feature groups, $G$, is the same, though in practice they often differ. We partition the row and column coefficient vectors $\boldsymbol{\alpha}$ and $\boldsymbol{\beta}$ into $\boldsymbol{\alpha}^{(g)}$ and $\boldsymbol{\beta}^{(g)}$ for groups $g=1,...,G$. 
...
\end{equation}
Now we can re-parameterize the joint optimization problem such that it is locally equivalent to
\begin{equation}
\begin{array}{c}
\min_{\boldsymbol{\lambda} \in \mathbb{R}^{2G+1}_{+}} \frac{1}{2|V|}
\left \| 
...
\end{equation}
where we use the same notational shorthand $\boldsymbol{X}_{T, I(\boldsymbol\lambda)}$ and $\boldsymbol{X}_{V, I(\boldsymbol\lambda)}$ from Section \ref{sec:enet}. This function is differentiable at its minimizer for almost every $\boldsymbol\lambda$.
...
\begin{align*}
\boldsymbol y &= \sum\limits_{i=1}^3 \boldsymbol f_i(\boldsymbol x_i) + \sigma \boldsymbol \epsilon\\
f_1(x) &= 9 \sin(3x)\\
f_2(x) &= x\\
