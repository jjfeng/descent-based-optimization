...
In our example, we consider jointly tuning all the penalty parameters, and in particular use group lasso penalties for the row and column features. This can be effective in settings where there is a natural grouping of the row and column features. For example, suppose we have standardized test scores from a student population. For each student, we have their homework scores from different subjects; for each test, we have features for different questions.
...
\end{equation}
with $\epsilon$ iid errors. Of this matrix $M$, we only observe some subset of the positions. Let the observed matrix positions be partitioned into a training set $T$, and a validation set $V$. Let $\| \cdot \|^2_T$ denote the Frobenius norm over the entries in $T$ and similarly define $\| \cdot \|^2_V$. For simplicity, we suppose there are the same number of row and column feature groups $G$. We partition the row and column coefficient vectors $\boldsymbol{\alpha}$ and $\boldsymbol{\beta}$ into $\boldsymbol{\alpha}^{(g)}$ and $\boldsymbol{\beta}^{(g)}$ for groups $g=1,...,G$. 
...
\})
$$ 
The differentiable space with respect to $\boldsymbol{\beta}$ is $span(\boldsymbol{I}_{\boldsymbol{I}_c(\boldsymbol\lambda)})$, which is defined similarly. Let $\{u_j\}_{j=1}^{\text{rank}(\hat{\boldsymbol{\Gamma}}(\boldsymbol{\lambda}))}$ denote a orthogonal basis of the column space of $\hat{\boldsymbol{\Gamma}}(\boldsymbol{\lambda})$. We show in the Appendix that the differentiable space with respect to $\boldsymbol{\Gamma}$ can be written as the span of an orthogonal basis $B_{\boldsymbol{\lambda}}^{(i)}$
\begin{align}
...
%\right \} \right )\\
 & = span \left (\left \{
B^{(i)}_{\boldsymbol{\lambda}}
\middle | 
i=1,...,K
\right \} \right )
\label{eq:nuclear_norm_diff_space}
\end{align}
where we let $K$ be the size of this orthogonal basis.
Putting these together, we have that the differentiable space for this problem is
\begin{equation}
...
\right )
\\
\text{ and } \boldsymbol{\Xi} = \sum_{i=1}^b b_i B^{(i)}_{\boldsymbol{\lambda}} 
\end{array}
