...
These problems often try to find low-rank matrix representations by minimizing the nuclear norm $\|\cdot \|_*$ \citep{fazel2002matrix, srebro2004learning}. The nuclear norm is non-smooth since it is the $\ell_1$ norm of the singular values.

In this example, we have a matrix-completion problem with additional information in the form of feature vectors for the rows and columns. We suppose that the matrix is the sum of row and column effects as well as an interaction matrix. Suppose the row and column effects are linear functions of the row and column feature vectors, respectively. \citep{fithian2013scalable} solves this problem with three penalty parameters -- one for the nuclear norm on the interaction matrix and one for each of the penalty functions on the row and column coefficients. Here we have row and column feature groups, and we would like to recover a sparse set of groups using group lasso penalties.

The exact problem setup is as follows. Suppose we are performing matrix-completion for the matrix $M \in \mathbb{R}^{N\times N}$. Let $X\in \mathbb{R}^{N \times p}$ be the feature vectors for the rows and $Z \in \mathbb{R}^{N \times p}$ be the feature vectors for columns.
...
%		\multicolumn{6}{|c|}{N=60, G=12, n=2}\\
		...
		\hline
		% TODO: RUNNING now: job 3935706
		% bayes: script_out/matrix_completion_groups_HC_11_new1.txt
		GD & 25 & ? \\
		\hline
		% TODO: RUNNING now: job 3935709
		% bayes: script_out/matrix_completion_groups_NM_11_new1.txt
		NM & 25 & ? & 100 \\
		\hline
		% TODO: RUNNING now: job 3935710
		% bayes: script_out/matrix_completion_groups_SP_11_new1.txt
		SP & 25 & ? & 100\\
		\hline
		% TODO: RUNNING now: job 3935707
...
%		\hline
%		% rerunning on bayes: script_out/matrix_completion_groups_HC_9.out
%		% bayes: script_out/matrix_completion_groups_runs_HC_9.txt
%		% OOPS:; script_out/matrix_completion_groups_GS_12.out
%		% OOPS:; script_out/matrix_completion_groups_HC_13.out
%		GD & 45 & 0.375 (0.014) & 0.377 (0.013)&  8.38 (0.57) & 27.28 (1.34) \\
%		\hline
%		% rerunning on bayes: script_out/matrix_completion_groups_GS_13.out
%		% bayes: script_out/matrix_completion_groups_runs_GS_9.txt
