...
In this example, we move away from the simple regression framework and consider matrix-valued data with partially observed entries. The goal in matrix-completion problems is to reconstruct the rest of the matrix. There are many applications where such problems come up. In collaborative filtering problems, the goal is to infer user ratings on products given user ratings on a subset of the products \citet{netflix}. In robust principal components analysis, we decompose the observed matrix into a low-rank matrix and a sparse ``noise'' matrix \citet{candes2011robust}. A common assumption throughout these problems is that the matrix is low-rank. One method to find such matrices is to minimize their nuclear-norm $\|\cdot \|_*$ \citep{fazel2002matrix, srebro2004learning}, which is defined as the sum of the singular values.
