...
When the penalized training criterion in the joint optimization problem is not smooth, gradient descent cannot be directly applied. Nonetheless, we find that in many problems, the solution $\hat{\boldsymbol \theta}\left(\boldsymbol{\lambda}\right)$ is smooth at almost every $\boldsymbol{\lambda}$ (eg. Lasso, Group Lasso, Trend Filtering \textcolor{red}{cite again!}); this means that we can indeed apply gradient descent in practice. In this section, we characterize these problems that are almost everywhere smooth. In addition, we provide a solution for deriving $\frac{\partial}{\partial \boldsymbol{\lambda}} \hat{\boldsymbol \theta}(\boldsymbol{\lambda})$ since calculating the gradient is a challenge in and of itself. This is then incorporated into an algorithm for tuning $\boldsymbol{\lambda}$ using gradient descent.
...
%In collaborative filtering problems, the goal is to infer user ratings on products given user ratings on a subset of the products \citet{netflix}. In robust principal components analysis, we decompose the observed matrix into a low-rank matrix and a sparse ``noise'' matrix \citet{candes2011robust}. 
In these problems it is popular to assume a low rank structure and reconstruct the matrix by minimizing a penalized loss with a nuclear norm penalty $\|\cdot \|_*$ \citep{fazel2002matrix, srebro2004learning}. The nuclear norm -- the sum of the singular values of a matrix -- is the matrix-variate analog to the lasso. It is non-smooth function and employing it as a penalty results in estimates that are low rank (many $0$ singular values).

There are several extensions to the matrix completion problem that allow one to incorporate additional information. We consider the work of \citet{fithian2013scalable}: Their formulation supposes one has additional covariates for each row and column, and models each entry of the matrix as a combination of a low rank effect, and a linear function of the corresponding column and row covariates. The attack this by posing a penalized loss; with both a nuclear norm penalty, and penalties on both the column and row coefficients. We consider jointly tuning all penalty parameters, and in particular use a combination of Lasso, Group-Lasso, Ridge, and nuclear norm penalties \textcolor{red}{(is this right?)}
...
\end{equation}
with $\epsilon$ iid errors. Of this matrix $M$, we only observe some subset of the positions. Let the observed matrix positions be partitioned into a training set $T$, and a validation set $V$. Let $\| \cdot \|^2_T$ denote the Frobenius norm over the entries in $T$ and similarly define $\| \cdot \|^2_V$. In addition, suppose the row and column features are composed of $G$ groups \textcolor{red}{$G$ groups each or total? Why might we have groups? This needs a clear concise sentence of motivation? It looks like you have the same groups for rows and columns. Why would we have that?}. We partition the row and column coefficient vectors $\boldsymbol{\alpha}$ and $\boldsymbol{\beta}$ into $\boldsymbol{\alpha}^{(g)}$ and $\boldsymbol{\beta}^{(g)}$ for groups $g=1,...,G$. 
...
The differentiable space for $\boldsymbol{\alpha}$ is $span(\boldsymbol{I}_{\boldsymbol{I}_r(\boldsymbol\lambda)})$ where
$$
...
\})
$$ 
The differentiable space for $\boldsymbol{\beta}$, denoted $span(\boldsymbol{I}_{\boldsymbol{I}_c(\boldsymbol\lambda)})$, is defined similarly. The differentiable space for $\Gamma$ is the set of all matrices that have the same rank \textcolor{red}{(That have the same rank as $\Gamma$? Can we move in all those directions? I don't think we can, I think we need the singular vectors to actually be a smooth perturbation of the singular vectors of $\Gamma$)}. This follows from the fact that the nuclear norm is a lasso penalty over the singular values \textcolor{red}{(this isn't really a "fact"... are you giving the intuition? or actually giving a justification?)}. For more details, refer to the appendix. Putting this together, we have
\begin{align}
...
To calculate the gradient of the validation loss, we need to slightly modify the steps outlined in Algorithm \ref{alg:gradDescent}. The details are given in the Appendix.
...
For this experiment, we considered a $60 \times 60$ matrix. The training set contained roughly two observed entries from each row and column and the validation set contained one per row and column. The row and column feature vectors contained 12 covariate groups each, where each group contained three covariates.
