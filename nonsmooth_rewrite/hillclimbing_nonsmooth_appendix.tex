...
\begin{itemize}
	...
	\item[] Condition 2: The nuclear norm penalty and the group lasso penalties are twice-differentiable when restricted to $S_{\boldsymbol{\lambda}}$.\hfill \ding{51}
	\item[] Condition 3: The Hessian of the training criterion is positive definite for any $\epsilon > 0$ since the nuclear norm and the group lasso penalties are convex and the frobenius norm and the ridge penalties are positive definite. \hfill \ding{51}
\end{itemize}
...
\begin{table}
	...
	\begin{tabular}{| l | l | l | l | l | l |}
		...
		\hline
		% rerunning with diff settings - cox: script_out/matrix_completion_groups_HC0_11.out
...
%		& \# $\lambda$ & Validation Err & Test Err&  $\Gamma$ Rank & Num Solves\\
%		\hline
%		% rerunning on bayes: script_out/matrix_completion_groups_HC0_9.out
