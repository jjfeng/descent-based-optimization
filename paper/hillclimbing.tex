...
Based on this theorem, we provide the gradient descent procedure for solving this class of joint optimization problems. For each iteration, the key idea is to define a smooth, locally equivalent function $\tilde{\theta}(\boldsymbol{\lambda})$ and use $\frac{\partial}{\partial \boldsymbol{\lambda}} \tilde{\theta}(\boldsymbol{\lambda})$ to calculate the gradient instead.
...
To better understand this procedure, we consider an example joint optimization problem with the lasso penalty
\begin{equation}
\begin{array}{c}
...
\label{lassoinner}
\end{equation}
At every iteration $k$, we consider the regression problem restricted to the space spanned by the non-zero elements of $\hat{\boldsymbol{\theta}}(\lambda^{(k)})$
\begin{equation}
...
Assuming that $(\boldsymbol{X}_T^T\boldsymbol{X}_T)^{-1}$ exists, \eqref{gradientCalculation} gives us 
\begin{equation}
\frac{\partial}{\partial \lambda} \tilde{\boldsymbol{\theta}}(\lambda) \big |_{\lambda = \lambda^{(k)}} = (\boldsymbol{X}_T^T\boldsymbol{X}_T)^{-1} sgn \left (\tilde{\boldsymbol{\theta}} (\lambda^{(k)}) \right )
\end{equation}
(Note that if $\boldsymbol{X}_T^T\boldsymbol{X}_T$ is not invertible, one can simply add a ridge penalty with a tiny fixed parameter to the training criterion.) Hence, the gradient descent direction for the $k$th iteration is
\begin{equation}
\nabla_{\lambda} L(\boldsymbol{y_V}, f_{\hat{\boldsymbol{\theta}}(\lambda)}(\boldsymbol{X_V})) |_{\lambda = \lambda^{(k)}} =
- \left (\boldsymbol{X}_V(\boldsymbol{X}_T^T\boldsymbol{X}_T)^{-1} sgn(\tilde{\boldsymbol{\theta}}(\lambda^{(k)})) \right )^T
