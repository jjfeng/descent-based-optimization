...
\frac{\partial}{\partial f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V)} L ( \boldsymbol{y}_V, f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V))^T 
\frac{\partial}{\partial \boldsymbol{\lambda}} f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V)
\label{chainrule}
\end{equation}
For most problems, calculating the derivative of the validation loss
$\frac{\partial}{\partial f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V)} L ( \boldsymbol{y}_V, f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V))$
is straightforward. Hence, we focus on the second term, the Jacobian matrix for the model parameters
$f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V)$
with respect to the regularization parameters $\boldsymbol{\lambda}$. Recall that $\hat{\theta}(\boldsymbol{\lambda})$ is the minimizer of the training set criterion. Therefore, the gradient of the training set criterion at $\hat \theta(\boldsymbol{\lambda})$ must be zero:
\begin{equation}
...
\end{equation}
We can therefore get the desired Jacobian matrix $\frac{\partial}{\partial \boldsymbol{\lambda}} f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V)$ by taking the implicit derivative of the gradient condition in  (\ref{traininggradientcondition}).

By using implicit differentiation, $\frac{\partial}{\partial \boldsymbol{\lambda}} f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V)$ will be in terms of $\hat{\theta}(\boldsymbol{\lambda})$, which means that each iteration of gradient descent requires solving the inner optimization problem. While this may seem costly, we can use warm starts to efficiently solve the inner problem. In the special case where there is a closed-form solution for $\hat{\theta}(\boldsymbol{\lambda})$, we can directly calculate the gradient and bypass solving the inner optimization problem. Ridge regression is one such example.
...
We make two primary assumptions to ensure that the validation loss is differentiable almost everywhere. Since the validation loss is usually a differentiable function with respect to the fitted values $f_{\hat \theta}(X_V)$, our assumptions only concern the inner optimization problem of the training loss. For notational convenience, we use $L_T(\theta, \boldsymbol{\lambda})$ to denote the training set criterion.

First, we require that the training criterion is twice continuously differentiable along some set of directions almost everywhere. To state this assumption more formally, we define the ``differentiable space" of a function at a point as follows:
...
\end{equation}
\end{definition}
Our first assumption is that the training loss $L_T(\theta, \boldsymbol{\lambda})$ has some differentiable space (of nonzero measure) at almost every point $(\theta, \boldsymbol{\lambda})$. For example, linear regression with the lasso penalty satisfies this requirement since the differentiable space with respect to the regression coefficients is the space spanned by the non-zero coefficients.
...
We assume that for almost every point $\lambda_0$, the differentiable space of the training loss $\Omega^{(L_T(\cdot, \boldsymbol{\lambda}))}(\hat \theta(\lambda_0))$ is also a local optimality space of the training loss $L_T(\cdot, \boldsymbol{\lambda})$ for some neighborhood containing $\lambda_0$. Intuitively, we are requiring that for two, nearby regularization parameters, the minimizers of the training loss lie in the same, smooth space. While this assumption may seem restrictive, it actually encompasses many common penalties. For example, in lasso regression, there are a finite number of $\lambda$ values where the regression coefficients enter the active set. At all other values $\lambda$, the set of non-zero regression coefficients is locally constant. In other words, for almost every $\lambda$, the space spanned by the non-zero coefficients of $\hat \theta(\lambda)$ is a local optimality space, which is exactly the differentiable space $\Omega^{(L_T(\cdot, \lambda))}(\hat \theta(\lambda))$.

Combining the two assumptions, we can conclude that the minimizer of the training loss is a continuously differentiable function of the regularization parameters, as stated in the following theorem. The proof is provided in the appendix.
...
From the following gradient condition, the Jacobian of $\dot \beta(\lambda^{(k)})$ can be found using implicit differentiation
\begin{equation}
0 = -\boldsymbol{X}_T^T(\boldsymbol{y}_T
- \boldsymbol{X}_T \hat{\boldsymbol{\beta}} (\lambda^{(k)}) )
+ \lambda^{(k)} sgn(\hat{\boldsymbol{\beta}} (\lambda^{(k)}))
\end{equation}
...
\begin{proof}
It is sufficient to show that the theorem holds for a given $\lambda_0$. For a given $\lambda_0$, let $B'$ be the basis vectors that span the differentiable space $\Omega^{L_T}(\hat \theta(\lambda_0), \lambda_0)$. Let $B$ be the subset of the basis vectors that span the space of $\theta$ vectors.
