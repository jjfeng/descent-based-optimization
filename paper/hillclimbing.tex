...
For reference, the differentiable space of each regression example is specified in Table \ref{table:differentiableSpace}.
...
\hline
Ridge Regression & $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}(\hat{\boldsymbol \theta}\left(\boldsymbol{\lambda}\right)) = \mathbb{R}^p$\\
\hline
Elastic Net & $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}(\hat{\boldsymbol \theta}\left(\boldsymbol{\lambda}\right)) = span(\{e_i | \hat{\theta}_i\left(\boldsymbol{\lambda}\right) \ne 0 \})$\\
\hline
Sparse Group Lasso & $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}(\hat{\boldsymbol \theta}\left(\boldsymbol{\lambda}\right)) = span(\{e_i | \hat{\theta}_i\left(\boldsymbol{\lambda}\right) \ne 0 \})$ \\
\hline
Nesterov's Gradient Descent & $\Omega^{L_T(\cdot, \cdot, \boldsymbol{\lambda})}(\hat{ \boldsymbol \beta}\left(\boldsymbol{\lambda}\right), \hat{ \boldsymbol \theta}\left(\boldsymbol{\lambda}\right)) = \mathbb{R}^{p+n} \setminus span(\{e_i | \hat{\beta}_i\left(\boldsymbol{\lambda}\right) \ne 0 \} \})$\\
\hline
\end{tabular}
\end{center}
