...
\begin{definition}
Let $B = \{ b_1, ..., b_p \}$ be an orthonormal set of vectors in $\mathbb{R}^n$. Let $f$ be a real-valued function over $\mathbb{R}^n$ and suppose its second directional derivatives of $f$ with respect to the vectors in $B$ exist. The Hessian matrix of $f$ with respect to $B$ is
\begin{equation}
\nabla^2_B f(x) \in \mathbb{R}^{p\times p} =
\left (
\begin{array}{cccc}
\frac{\partial^2 f(x)}{\partial b_1^2} & \frac{\partial^2 f(x)}{\partial b_1 \partial b_2} & ...  & \frac{\partial^2 f(x)}{\partial b_1 \partial b_p} \\
\frac{\partial^2 f(x)}{\partial b_2 \partial b_1} & \frac{\partial^2 f(x)}{\partial b_2^2} & ...  & \frac{\partial^2 f(x)}{\partial b_2 \partial b_p} \\
\vdots & \vdots &  \ddots & \vdots \\
\frac{\partial^2 f(x)}{\partial b_p \partial b_1} & \frac{\partial^2 f(x)}{\partial b_p \partial b_2} & ...  & \frac{\partial^2 f(x)}{\partial b_p^2} \\
\end{array}
\right )
\end{equation}
\end{definition}
...
\begin{equation}
\begin{array}{c}
\min L(y_V, f_{U^{(k)} \hat{\boldsymbol \beta} (\boldsymbol \lambda^{(k)}) }(X_V)) \\
\text{where } \hat{\boldsymbol \beta} (\boldsymbol \lambda^{(k)}) =
\argmin_{\boldsymbol \beta}
L(y_T, f_{U^{(k)} \boldsymbol \beta}(X_T))
+ \sum\limits_{i=1}^J \lambda_i^{(k)} P_i( U^{(k)} \boldsymbol \beta)
\end{array}
\end{equation}
...
\end{equation}
Note that because the differentiable space is assumed to be locally constant, we treat $U^{(k)}$ as a constant in the gradient derivations. Below, we give the descent procedure in more detail.
  ...
  \fbox{
    \parbox{48em}{
        ...
        \begin{itemize}
              ...
              \item[] Construct matrix $U^{(k)}$ where its columns are an orthonormal basis of $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}\left (\hat{\theta}\left(\boldsymbol{\lambda}^{(k)}\right) \right )$.
		...
		= - \left [
		U^{(k)T}
		\nabla_{\boldsymbol \gamma}^2 \left (
			 L(\boldsymbol{y}_T, f_{\boldsymbol \gamma} (\boldsymbol{X}_T))  + 
			 \sum\limits_{i=1}^J \lambda_i P_i(\boldsymbol \gamma)
		\right )
		\bigg |_{\boldsymbol \gamma = U^{(k)} \hat{\boldsymbol \beta}(\boldsymbol \lambda)}
		U^{(k)}
		\right ]^{-1}
		U^{(k)T}
		\left [
		\nabla_{\boldsymbol \gamma} P(\boldsymbol \gamma)
		|_{\boldsymbol \gamma = U^{(k)} \hat{\boldsymbol \beta}(\boldsymbol \lambda)}
		\right ]
              \end{equation}
...
\begin{equation}
\begin{array}{c}\hat{\boldsymbol{\beta}}
...
\begin{equation}
\nabla_{\lambda} L(\boldsymbol{y_V}, f_{\hat{\boldsymbol{\beta}}(\lambda)}(\boldsymbol{X_V})) = 
- \left (
X_V U_\lambda
\frac{\partial}{\partial \lambda} \hat{\boldsymbol \beta}(\lambda)
\right )^T
