  ...
  \fbox{
    \parbox{48em}{
        ...
        \begin{itemize}
              ...
              \item[] Find the projection matrix $M_{\boldsymbol{\lambda}^{(k)}}$ onto $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}\left (\hat{\theta}\left(\boldsymbol{\lambda}^{(k)}\right) \right )$ with rank $q$. Let $U_{\boldsymbol{\lambda}^{(k)}}$ be the $p \times q$ matrix  such that $M_{\boldsymbol{\lambda}^{(k)}} = U_{\boldsymbol{\lambda}^{(k)}} U_{\boldsymbol{\lambda}^{(k)}}^T$ and $U_{\boldsymbol{\lambda}^{(k)}}^T U_{\boldsymbol{\lambda}^{(k)}} = I_q$.
              
              \item[] Define the locally equivalent joint optimization problem
              \begin{equation}
              \begin{array}{c}
              \min L(y_V, f_{M_{\boldsymbol \lambda^{(k)}} \tilde{\boldsymbol \theta}(\boldsymbol \lambda)}(X_V)) \\
              \tilde{\theta}(\boldsymbol{\lambda}^{(k)}) =
              \argmin_{\theta}
              L(y_T, f_{M_{\boldsymbol \lambda^{(k)}} \theta}(X_T))
