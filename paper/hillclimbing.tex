...
Many joint optimization problems can be solved using gradient descent. The procedure is relatively simple, at least in principle. To find the regularization parameters $\lambda$ that minimize the validation criterion $L \Big( \boldsymbol{y}_V, f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V) \Big )$,
...
\end{equation}
Our approach is to find a locally equivalent function $\tilde{\theta}(\boldsymbol{\lambda})$ that is the minimizer of the training criterion over a subspace $\tilde \Theta(\lambda)$ in which the training criterion is twice continuously differentiable. That is, for a given $\lambda$, we find some neighborhood $W$ containing $\lambda$ such that for every $\lambda' \in W$
\begin{equation}
...
Finding the gradient descent direction is straightforward once we have the twice differentiable function $\tilde \theta(\lambda)$. To find $\frac{\partial}{\partial \lambda} \hat{\theta}(\boldsymbol{\lambda})$, we perform implicit differentiation on the gradient condition
\begin{equation}
...
\begin{equation}
\frac{\partial}{\partial \lambda} \tilde{\theta}(\boldsymbol{\lambda}) = 
\Big [ \nabla_\theta^2 \big (  L(\boldsymbol{y}_T, f_\theta (\boldsymbol{X}_T))  +  \sum\limits_{i=1}^J \lambda_i P_i(\theta)  \big )  \Big ]^{-1} \Big (\nabla_\theta P_1(\theta) \Big | \nabla_\theta P_2(\theta)\Big |  ... \Big |  \nabla_\theta P_J(\theta) \Big ) \bigg |_{\theta = \tilde \theta(\boldsymbol{\lambda})}
\label{implicitDifferentiation}
\end{equation}
%% CHECK ABOVE EQUATION
Plugging (\ref{implicitDifferentiation}) into (\ref{chainrule}) gives us the gradient descent direction for our procedure.
...
\begin{equation}
\Omega^{(L)}(\eta) = \text{span} \Big(\Big\{ \delta | \lim_{\epsilon \rightarrow 0} \frac{L(\eta + \epsilon \delta) - L(\eta)}{\epsilon} \Big\}\Big)
\end{equation}
\end{definition}
...
\begin{equation}
L_T(\theta, \lambda) = L(\boldsymbol{y}_T, f_\theta (\boldsymbol{X}_T)) + \sum\limits_{i=1}^J \lambda_i P_i(\theta)
\end{equation}
First, we require that the differentiable space of $L_T(\cdot, \lambda)$ at $\hat \theta(\lambda)$ is a local optimality space for some neighborhood $W$ around $\lambda$. This condition formalizes the previously mentioned $\tilde \Theta(\lambda)$ and ensures that we can always restrict gradient descent to a smooth space.

For example, the lasso regression satisfies this condition. As $\lambda$ increases, the lasso regression coefficients $\hat \theta(\lambda)$ are set to zero. Since the lasso penalty is defined as 
\begin{equation}
P(\theta) = \sum_{i=1}^{p} | \theta_i |
\end{equation}
the differentiable space of $L_T(\cdot, \lambda)$ at $\hat \theta(\lambda)$ is only differentiable along directions spanned by the nonzero elements of $\hat \theta(\lambda)$. Furthermore, there exist a finite number of $\lambda$ values such that elements of $\hat \theta(\lambda)$ become zero as $\lambda$ increases, which means that within some neighborhood $W$ of $\lambda$, the nonzero elements of $\hat \theta(\lambda)$ stay constant. Therefore, the differentiable space is a local optimality space. Other joint optimization problems that satisfy this condition include the generalized lasso and all the examples used in this paper.

The two other conditions ensure that $\frac{\partial}{\partial \lambda} \tilde \theta(\lambda)$ exists. For (\ref{implicitDifferentiation}) to be valid, we need $L_T$ to be twice continuously differentiable and $\nabla_\theta^2 L_T(\theta, \lambda)$ to be invertible almost everywhere. As a side note, in cases where $\nabla_\theta^2 L_T(\theta, \lambda)$ is singular, such as the lasso for high-dimensional datasets, one can simply add a ridge penalty with a small coefficient.
...
where
\begin{equation}
\tilde{\theta}(\boldsymbol{\lambda}) = \arg \min_{\theta \in \Omega^{(\boldsymbol{\lambda})}} L_T(\theta , \boldsymbol{\lambda})
...
\label{lassoGradient}
\end{equation}
and implicit differentiation of (\ref{lassoGradient}) gives us
\begin{equation}
