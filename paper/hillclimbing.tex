...
We can plug \eqref{implicitDifferentiation} into \eqref{chainrule} to get $\nabla_{\boldsymbol{\lambda}} L \Big( \boldsymbol{y}_V, f_{\hat{\theta}(\boldsymbol{\lambda})}(\boldsymbol{X}_V) \Big )$. Note that because $\frac{\partial}{\partial \boldsymbol{\lambda}} \hat{\theta}(\boldsymbol{\lambda})$ is defined in terms of $\hat{\theta}\left(\boldsymbol{\lambda}\right)$, each gradient step requires solving the penalized regression problem on the training data. Below, we present the updated version of Algorithm 1 with the specific gradient calculations:
...
We can therefore construct a gradient descent procedure based on the key idea in \eqref{restrictedmodelparams}. At each iteration, let matrix $U_{\boldsymbol \lambda^{(k)}}$ have orthonormal columns spanning the differentiable space of the training loss at $\boldsymbol \lambda^{(k)}$. Since the differentiable space is also a local optimal space, it is sufficient to minimize the training criterion over the column span of $U_{\boldsymbol \lambda^{(k)}}$. We formulate this locally equivalent joint optimization problem as follows:
\begin{equation}
\begin{array}{c}
\min L(y_V, f_{U_{\boldsymbol \lambda^{(k)}} \tilde{\boldsymbol \theta}(\boldsymbol \lambda^{(k)})}(X_V)) \\
\text{where } \tilde{\theta}(\boldsymbol \lambda^{(k)}) =
\argmin_{\theta \in C \left (U_{\boldsymbol \lambda^{(k)}} \right )}
L(y_T, f_{U_{\boldsymbol \lambda^{(k)}} \theta}(X_T))
+ \sum\limits_{i=1}^J \lambda_i P_i( U_{\boldsymbol \lambda^{(k)}} \theta)
\end{array}
\end{equation}

This problem now reduces to the simple case where the training criterion is smooth. As mentioned previously, we can simply perform implicit differentiation on the gradient condition. Note that because the differentiable space is assumed to be locally constant, $U_{\boldsymbol \lambda^{(k)}}$ can be treated as a constant. Below, we give the descent procedure in more detail.
  ...
  \fbox{
    \parbox{48em}{
        ...
        \begin{itemize}
              ...
              \item[] Construct matrix $U_{\boldsymbol{\lambda}^{(k)}}$ where its columns are the orthonormal basis of $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}\left (\hat{\theta}\left(\boldsymbol{\lambda}^{(k)}\right) \right )$.
              ...
              \item[] Calculate $\frac{\partial}{\partial \boldsymbol \lambda} \tilde{\theta}(\boldsymbol{\lambda})|_{\boldsymbol{\lambda} = \boldsymbol{\lambda}^{(k)}}$ where
              \begin{equation}
	      \frac{\partial \tilde{\theta}(\boldsymbol{\lambda})}{\partial \boldsymbol \lambda} 
		= - \left [
		\nabla_{\theta}^2 \left (
			 L(\boldsymbol{y}_T, f_{U_{\boldsymbol \lambda^{(k)}} \theta} (\boldsymbol{X}_T))  + 
			 \sum\limits_{i=1}^J \lambda_i P_i(U_{\boldsymbol \lambda^{(k)}} \theta)
		\right )
		\right ]^{-1}
		\nabla_\theta P(U_{\boldsymbol \lambda^{(k)}} \theta)
              \end{equation}
              ...
              \left [
              	\nabla_\theta L(\boldsymbol{y_V}, f_{U_{\boldsymbol \lambda^{(k)}} \theta}(\boldsymbol{X_V}))
              \right ] \Big |_{\boldsymbol \theta = \tilde{\boldsymbol \theta}(\boldsymbol \lambda)}^T
...
To better understand the proposed gradient descent procedure, we present example joint optimization problems and their corresponding gradient calculations. We start with ridge regression where the training criterion is smooth. Then we consider the elastic net, sparse group lasso, and the generalized lasso, in which the training criterions are smooth almost everywhere. Finally, we discuss doing descent-based joint optimization for an additive partial linear model, an example of a semi-parametric regression.

For reference, the differentiable space of each regression example is specified in Table \ref{table:differentiableSpace}. For ease of notation, we will let $S_{\boldsymbol{\lambda}}$ denote the differentiable space of $L_T(\cdot, \boldsymbol{\lambda})$ at $\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})$.
...
Let the nonzero indices of $\hat{\boldsymbol{\theta}}$ be denoted $I(\boldsymbol\lambda) = \{i | \hat{\boldsymbol{\theta}}_i(\boldsymbol\lambda) \ne 0\}$ and let $I_{I(\boldsymbol \lambda)}$ be a submatrix of the identity matrix with columns $I(\boldsymbol\lambda)$. Since the absolute value function is not differentiable at zero, the directional derivative of $||\boldsymbol \theta||_1$ only exists along directions spanned by the columns of $I_{I(\boldsymbol \lambda)}$. That is, the differentiable space at $\boldsymbol \lambda$ is
\begin{equation}
S_{\boldsymbol{\lambda}} = C(I_{I(\boldsymbol \lambda)})
\end{equation}

Next, we show that the joint optimization problem satisfies all three conditions in Theorem \ref{thethrm}.
First, note that the nonzero indices of the regression coefficients from elastic net stay locally constant for almost every $\boldsymbol{\lambda}$. Therefore, $S_{\boldsymbol{\lambda}}$ is a local optimality space for $L_T(\cdot, \boldsymbol{\lambda})$ and the first condition is satisfied.
The second condition is also satisfied since the $\ell_1$ penalty is smooth when restricted to $S_{\boldsymbol{\lambda}}$.
Finally, the Hessian matrix of $L_T(\cdot, \boldsymbol{\lambda})$ with respect to the columns of $I_{I(\boldsymbol \lambda)}$ is $I_{I(\boldsymbol \lambda)}^T \boldsymbol{X}_{T}^T \boldsymbol{X}_{T} I_{I(\boldsymbol \lambda)} + \lambda_2 \boldsymbol{I}$. Since this is almost always invertible in practice, the third condition is satisfied.

To calculate the gradient, we consider the locally equivalent joint optimization problem
\begin{equation}
\begin{array}{c}
\min_{\boldsymbol{\lambda} \in \mathbb{R}^2} \frac{1}{2} \lvert\lvert \boldsymbol{y}_V - \boldsymbol{X}_V \boldsymbol{I}_{I(\boldsymbol \lambda)} \tilde{\boldsymbol{\theta}} (\lambda) \rvert\rvert ^2 \\
\text{ where }
\tilde{\boldsymbol{\theta}} (\boldsymbol{\lambda}) = \argmin_{\boldsymbol{\theta} \in C(\boldsymbol{I}_{I(\boldsymbol \lambda)})} \frac{1}{2} \lvert\lvert \boldsymbol{y}_T - \boldsymbol{X}_T \boldsymbol{I}_{I(\boldsymbol \lambda)} \boldsymbol{\theta} \rvert\rvert ^2
+ \lambda_1 \lvert\lvert \boldsymbol{I}_{I(\boldsymbol \lambda)} \boldsymbol{\theta} \rvert \rvert_1
+ \frac{1}{2}\lambda_2 \lvert\lvert \boldsymbol{I}_{I(\boldsymbol \lambda)} \boldsymbol{\theta} \rvert \rvert_2^2
\end{array}
\end{equation}
...
\begin{equation}
\begin{array}{c}
\min_{\boldsymbol{\lambda} \in \mathbb{R}^2} \frac{1}{2} \lvert\lvert \boldsymbol{y}_V - \boldsymbol{X}_{V, I(\boldsymbol \lambda)} \tilde{\boldsymbol{\theta}} (\lambda) \rvert\rvert ^2 \\
\text{ where }
\tilde{\boldsymbol{\theta}} (\boldsymbol{\lambda}) = \argmin_{\boldsymbol{\theta}} \frac{1}{2} \lvert\lvert \boldsymbol{y}_T - \boldsymbol{X}_{T, I(\boldsymbol \lambda)} \boldsymbol{\theta} \rvert\rvert ^2
+ \lambda_1 \lvert\lvert \boldsymbol{\theta} \rvert \rvert_1
+ \frac{1}{2}\lambda_2 \lvert\lvert \boldsymbol{\theta} \rvert \rvert_2^2
\end{array}
\end{equation}
...
As before, let the nonzero indices of $\hat{\boldsymbol{\theta}}$ be denoted $I(\boldsymbol\lambda) = \{i | \hat{\boldsymbol{\theta}}_i(\boldsymbol\lambda) \ne 0\}$. Since the directions for which $||\cdot||_2$ and $||\cdot||_1$ are differentiable are the same, the differentiable space is spanned by the nonzero elements of $\hat{\boldsymbol\theta}$:
\begin{equation}
S_{\boldsymbol \lambda} = C(\boldsymbol I_{I(\boldsymbol\lambda)})
\end{equation}
...
\begin{equation}
\begin{array}{c}
\min_{\boldsymbol{\lambda} \in \mathbb{R}^2} \frac{1}{2n} \bigl\lvert\bigl\lvert \boldsymbol{y}_V - X_{V,I(\boldsymbol\lambda)} \hat{\boldsymbol{\theta}}(\boldsymbol{\lambda}) \bigl\rvert\bigl\rvert^2_2 \\
\text{ where }
...
\end{bmatrix}
\end{equation}
Matrices $\boldsymbol{M}_1(\boldsymbol\lambda)$ and $\boldsymbol{M}_2(\boldsymbol\lambda)$ are defined in the Appendix.
...
\begin{equation}
\begin{array}{c}
\min_{\lambda} \frac{1}{2} \lvert\lvert \boldsymbol{y}_V - \boldsymbol{X}_V U_\lambda \tilde{\boldsymbol{\theta}} (\lambda) \rvert\rvert ^2 \\
\text{ where }
\tilde{\boldsymbol{\theta}} (\lambda) =
\argmin_{\boldsymbol{\theta} \in C(U_\lambda)}
\frac{1}{2} \lvert\lvert \boldsymbol{y}_T - \boldsymbol{X}_T U_\lambda \boldsymbol{\theta} \rvert\rvert ^2
+ \lambda \lvert\lvert D U_\lambda \boldsymbol{\theta} \rvert \rvert_1
\end{array}
\label{genlassoeasy}
\end{equation}

Since the training criterion in \eqref{genlassoeasy} is differentiable with respect to $\theta$, we have the gradient condition
\begin{equation}
- (\boldsymbol{X}_T U_\lambda)^T (\boldsymbol{y}_T - \boldsymbol{X}_T U_\lambda \boldsymbol{\theta})
+ \lambda (D U_\lambda)^T sgn(DU_\lambda \tilde{\boldsymbol \theta}(\lambda)) = 0
...
\begin{equation}
\frac{\partial}{\partial \lambda} \tilde{\boldsymbol \theta}(\lambda) =
-(U_\lambda^T X_T^T X_T U_\lambda)^{-1} U_\lambda^T D^T sgn(DU_\lambda \tilde{\boldsymbol \theta}(\lambda))
...
\nabla_{\boldsymbol \lambda} L(\boldsymbol{y_V}, f_{\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})}(\boldsymbol{X_V})) = 
- \left (
\boldsymbol{y}_V - \boldsymbol{X}_V U_\lambda \tilde{\boldsymbol{\theta}} (\lambda)
\right )^T
X_V U_\lambda
\frac{\partial}{\partial \lambda} \tilde{\boldsymbol \theta}(\lambda)
\end{equation}
...
\bigl\lvert\bigl\lvert
\boldsymbol{y}_V
- \boldsymbol{X}_{V, I(\boldsymbol \lambda)} \hat{\boldsymbol{\beta}}(\boldsymbol{\lambda})
- (\boldsymbol{I} - \boldsymbol{M}) \hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})
\bigl\rvert\bigl\rvert^2_2 \\
\text{ where }
...
Since this training criterion is smooth, we can perform implicit differentiation on the gradient condition and solve for $\frac{\partial}{\partial \boldsymbol \lambda} \tilde{\boldsymbol\beta}(\boldsymbol{\lambda}) $ and $\frac{\partial}{\partial \boldsymbol \lambda} \tilde{\boldsymbol\theta}(\boldsymbol{\lambda}) $: 
\begin{equation*}
\begin{array}{lcl}
