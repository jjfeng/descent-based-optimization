...
\end{center}
\caption{Left: A hypothetical grid of points that an exhaustive grid search would fit models for. The darkness of each point indicates the validation cost; dark points mean lower cost. In this example, descent-based optimization may takes steps along the arrow, a path that directly goes towards the minimizer of the criterion. Right: A 3D version illustrating the same situation.}
\label{fig:compare}
\end{figure}
...
\hline
Generalized Lasso & $\mathcal{N}(I_{I(\lambda)} D)$ where $I(\lambda) = \{ i | (D \hat{\boldsymbol{\theta}}(\lambda))^\top e_i = 0  \text{ for } i=1,...,p \}$ \\
\hline
