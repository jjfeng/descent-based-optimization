...
\textcolor{red}{Perhaps we should just write this problem (and PLAM) with a tiny additional ridge penalty? That will also give us uniqueness of the solution, which would be nice =]}
...
This problem satisfies the first two conditions in Theorem \ref{thethrm}. The third condition requires that the Hessian matrix is invertible. It is not guaranteed to hold. However, if one adds a small ridge penalty then Hessian matrix is invertible.
...
The first two conditions in Theorem 1 \textcolor{red}{You should always use a label/ref combo to reference theorems and algorithms} are satisfied by similar reasoning to that discussed in Section~\ref{sec:net}. For the third condition, we need to check that the Hessian matrix is invertible.
\begin{itemize}
...
\end{equation}
In settings with enough data, the Hessian will be nonsingular in practice. In a high-dimensional, low data setting ($p \gg n$), the Hessian may not be. This can be avoided by simply adding a ridge penalty with a small regularization parameter. Therefore we can presume that Condition 3 is satisfied. \textcolor{red}{our life may be easier if we just add a tiny bit of ridge to everything to being with}
\end{itemize}
...
\end{bmatrix}
\end{equation}
The Hessian matrix is invertible for $\lambda_2 > 0$. \textcolor{red}{Not quite! But if we add a tiny ridge penalty to $\beta$ then I think we are ok. And we are definitely OK if we add a tiny ridge penalty to both}
\end{itemize}
...
There are many possible choices for our stepsize sequence $\{\alpha_k\}$. Popular choices for convex problems are discussed in \textcolor{red}{CITE BOYD}. We chose use a backtracking line search as discussed in \textcolor{red}{CHAPTER of CITE BOYD}. In our examples initial step size was between 0.5 and 1 and we backtrack at a rate between 0.1 and 0.01. \textcolor{red}{Maybe give details in the appendix for what this means? Or give it here, your choice!} \textcolor{blue}{Also should note what we do if our $\lambda$s try to leave the positive quadrant --- that we automatically discard those step-sizes}
...
\]
for some prespecified tolerance, $\epsilon$. For the results in this manuscript we use $\epsilon = BLAH$.

\subsection{Accelerated Gradient Descent}
We also use a modification of Algorithm 2, based on the work of \textcolor{red}{CITATIONS}. For smooth convex problems, these ``accelerated'' algorithms have faster worst-case convergence than gradient (while maintaining the same per-iteration complexity). In practice, these accelerated algorithms often vastly improve performance. In particular we follow the recipe of \citet{o2013adaptive} \textcolor{red}{Do they have step size optimization in there? Do we use step size optimization?}. Our algorithm is given by:\textcolor{red}{ALGORITHM GOES HERE --- be more explicit than you were below though!}

\subsection{OLD SECTION}
\textcolor{red}{leaving this for comparison, and in case you want to cannabalize bits f the algorithm for above}
...
\textcolor{red}{There should be something in here about how we fit the inner optimizations / fit the solutions at all grid-points for grid-based optimization; i.e. mention using cvxpy and whichever solver we specified in cvxpy --- you can note that other custom-tuned solvers would be faster, BUT since both methods had this disadvantage we don't expect it to influence the relative timings}
