...
We now present the full modified gradient descent:
  ...
  \fbox{
    \parbox{43em}{
        ...
        \begin{itemize}
              ...
              \item[] Let $\Omega^{(k)}$ be the differentiable space of the training set criterion
              \begin{equation*}
              g(f |\boldsymbol{\lambda}^{(k)}) = L(\boldsymbol{y}_T, f (\boldsymbol{X}_T)) + \sum\limits_{i=1}^k \lambda_i^{(k)} J_i(f)
              \end{equation*}
              at $\hat f(\boldsymbol{\lambda}^{(k)})$.
              \item[] Calculate the maximum directional derivative of $\hat f(\boldsymbol{\lambda}^{(k)})$ in the subspace $\Omega^{(k)}$, denoted as $d_k$.
              \item[] Calculate the gradient
              \begin{equation*}
              \nabla_{\boldsymbol{\lambda}} L_V(\hat f(\boldsymbol{\lambda}^{(k)})) = \Big ( \frac{\partial}{\partial \hat f}L_V(\hat f(\boldsymbol{\lambda}^{(k)})) \Big )^T d_k
              \end{equation*}
