...
Descent-based and grid-based joint optimization had the same performance for all three situations. As shown in Table \ref{table:validation}, the only major difference between the different implementations of joint optimization is in the runtimes. In all the experiments, gradient descent was over 40\% than the grid-based approach and Nesterov's subgradient descent was over 50\% faster.
...
In this experiment, we test the effectiveness of descent-based joint optimization on an ``unpooled" variant of sparse group lasso. We increased the number of penalty parameters by replacing the group lasso penalty $\lambda_1$ with $m$ parameters $\{\lambda_1^{(k)}\}_{l=1,...,m}$ as follows:
