...
As mentioned before, we use cross-validation to find the optimal penalty parameters $\boldsymbol{\lambda} = (\lambda_1, ..., \lambda_k)^T$. Let the data set be partitioned into a training set $(\boldsymbol{y}_T , \boldsymbol{X}_T)$ and validation set $(\boldsymbol{y}_V, \boldsymbol{X}_V)$. The optimal penalty and model parameters are obtained by solving the following joint optimization problem:
\begin{equation}
\begin{array}{c}
...
We propose solving this joint optimization problem explicitly using a descent-based method over the regularization parameter space. In the simple case where the criterion is differentiable with respect to the penalty parameters, we can update the penalty parameters using gradient descent or some variant thereof. In fact, we show that gradient descent can be applied in situations where the criterion is not differentiable.

Our approach is different from grid-based methods, which do not incorporate knowledge about the criterion function. Grid-based methods solve the joint optimization problem by fitting models over a $k$-dimensional grid $G$ in the penalty parameter space. Therefore, the computational runtime of grid-based methods grows exponentially with the number of parameters.
...
\subsubsection{Differentiable almost everywhere! Complicated! (Work in progress...)}
...
\begin{equation}
0 \in \delta_{\hat f} \big (L(\boldsymbol y_T, \hat f (X_T; \Lambda)) + \sum\limits_{i=1}^k \lambda_i J_i(\hat f; \Lambda) \big)
\end{equation}
