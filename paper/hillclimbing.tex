...
\begin{equation}
\Omega^{(g)}(x) = \text{span} \Big(\{\delta | \lim_{\epsilon \rightarrow 0} \frac{g(x + \epsilon \delta) - g(x)}{\epsilon} \}\Big)
\end{equation}
\end{definition}
For example, the differentiable space of the lasso penalty function $||x||_1$ at the point $x_0$ is the space spanned by the non-zero elements of $x_0$. We use this ``differentiable space" concept to define the descent direction at each iteration of our algorithm.

\begin{definition}
The real-valued function $g(x,y)$ has ``local optimality space" $S$ at $x_0$ if there exists some neighborhood $B$ around $x_0$ such that for every $x\in B$, the minimizer of $g(x, \cdot)$ is in the space $S$:
\begin{equation}
\arg \min_y g(x, y) = \arg \min_{y \in S} g(x, y)
\end{equation}
\end{definition}

This concept of a ``local optimality space" will be required for specifying the assumptions required for this algorithm.

We propose solving joint optimization problems with a modified gradient descent that restricts the descent direction to a differentiable space. That is, at each iteration $k$, we solve for the model parameters $\hat f(\boldsymbol{\lambda}^{(k)})$, which determines the differentiable space of the training set criterion $L_T(f | \boldsymbol{\lambda})$ at these model parameters. We use the short-hand notation $\Omega^{(k)}$ to denote $\Omega^{(L_T)}(\hat f(\boldsymbol{\lambda}^{(k)}))$. To get the descent direction, we consider the modified optimization problem in which the model parameters are restricted to the space $\Omega^{(k)}$, as shown below:
...
To better understood this procedure, we consider an example joint optimization problem with the elastic net. At each iteration $k$, the inner optimization problem is
\begin{equation}
...
\end{equation}
where $s$ is the subdifferential of the absolute function. For the gradient step, we only need to calculate the gradient of the validation loss in which the regression coefficients $\hat \beta(\boldsymbol{\lambda}^{(k)})$ are restricted to the differentiable space $\Omega^{(k)}$. The subdifferential $s(\hat{\boldsymbol{\beta}} (\boldsymbol{\lambda}^{(k)}))$ is just the gradient with zeros for the zero elements in $\hat \beta(\boldsymbol{\lambda}^{(k)})$ and ones otherwise. Consequently, the KKT conditions reduce to equalities and we can get the gradient for tuning the regularization parameters using implicit differentiation.

This modified gradient descent relies on two key assumptions. The first assumption is that for almost every point $x$, the training criterion is twice continuously differentiable at $x$ along a set of directions. We use this assumption to show that it is sufficient to perform gradient descent instead of subgradient descent.

The second assumption is that the local optimality space of the training set criterion at the regularization parameters $\boldsymbol{\lambda}$ is the differentiable space at that point. Informally, we are requiring that nearby regularization parameters have model parameters that are within the same, smooth space. This allows us to guarantee that the descent direction for the restricted optimization problem and the original problem are the same. Although this assumption may seem unrealistic, many penalty functions satisfy this requirement. For instance, in lasso regression, the zero-valued regression coefficients for the penalty parameter $\lambda$ stay at zero within a neighborhood of $\lambda$. Therefore, the local optimality space at $\lambda$ is the space spanned by the nonzero regression coefficients, which is also the differentiable space of the lasso regression at $\lambda$.
...
\end{equation}
and let $\Omega^{(\boldsymbol{\lambda})}$ denote the differentiable space of $L_T(\cdot, \boldsymbol{\lambda})$ at $\hat f(\boldsymbol{\lambda})$.
Suppose that for almost every $\boldsymbol{\lambda}$, $L(\boldsymbol{y}, f(\boldsymbol{X}))$ and
$\sum\limits_{i=1}^k \lambda_i J_i(f)$ are twice continuously differentiable with respect to $f$ along directions in $\Omega^{(\boldsymbol{\lambda})}$ and the hessian of $L_T(f , \boldsymbol{\lambda})$ with respect to $f$ is nonsingular.
 In addition, suppose that for almost every $\boldsymbol{\lambda}$, the local optimality space of $L_T(f, \boldsymbol{\lambda})$ is $\Omega^{(\boldsymbol{\lambda})}$. Then the validation loss $L(\boldsymbol{y_V}, \hat f(\boldsymbol{X_V} | \boldsymbol{\lambda}))$ is continuously differentiable almost everywhere with respect to $\boldsymbol{\lambda}$ and its gradient, where it is defined, is
\begin{equation}
