  ...
  \fbox{
    \parbox{43em}{
        ...
        \begin{itemize}
              ...
              \begin{equation}
              \tilde{\theta}(\boldsymbol{\lambda}^{(k)}) = \argmin_{\theta \in \Omega^{L_T(\cdot, \boldsymbol{\lambda})}\left (\hat{\theta}\left(\boldsymbol{\lambda}^{(k)}\right) \right )} L_T(\theta, \boldsymbol{\lambda}^{(k)} )
              \end{equation}
...
Then the locally equivalent problem to \eqref{genlasso} at $\lambda$ is
\begin{equation}
\begin{array}{c}
...
Since $\tilde{\boldsymbol \theta}(\lambda)$ in \eqref{genlassoeasy} is differentiable, we can apply \eqref{implicitDifferentiation} to get
\begin{equation}
\frac{\partial}{\partial \lambda} \tilde{\boldsymbol \theta}(\lambda) =
-(X_T^T X_T M_\lambda)^{-1} (DM_\lambda)^T sign(DM_\lambda \tilde{\boldsymbol \theta}(\lambda))
\end{equation}

Therefore, the gradient of the validation loss with respect to $\lambda$ is
\begin{equation}
\nabla_{\boldsymbol \lambda} L(\boldsymbol{y_V}, f_{\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})}(\boldsymbol{X_V})) = 
\left (
X_V M_\lambda
(X_T^T X_T M_\lambda)^{-1} (D M_\lambda)^T sign(DM_\lambda \tilde{\boldsymbol \theta}(\lambda))
\right )^T
\left (
