...
\begin{definition}
$S$ is a ``local optimality space" for a convex function $L(\cdot, \lambda)$ at $\lambda_0$ if there exists a neighborhood $W$ containing $\lambda_0$ such that for every $\lambda \in W$,
\begin{equation}
...
%% Maybe mention that we are also doing a semi-parametric model example.
...
%\text{where } \hat{\boldsymbol{\theta}}(\lambda) = \arg \min_{\boldsymbol{\theta}} ||\boldsymbol{y}_T - \boldsymbol{X}_T \boldsymbol{\theta}||^2_2 + \lambda ||\boldsymbol{\theta}||_1
%\end{array}
%\label{lassoinner}
%\end{equation}
%
...
%\begin{equation}
%\tilde{\boldsymbol{\theta}}(\lambda^{(k)}) = \arg \min_{\{\boldsymbol{\theta} | \hat{\boldsymbol{\theta}}_i(\lambda^{(k)}) \ne 0 \} } ||\boldsymbol{y}_T - \boldsymbol{X}_T \boldsymbol{\theta}||^2_2 + \lambda^{(k)} ||\boldsymbol{\theta}||_1
%\end{equation}
%
...
First, we note that this problem satisfies the three conditions given in theorem \ref{thethrm}. The reasoning is very similar to that in the elastic net example so we do not repeat them here. Since $||\cdot||_2$ and $||\cdot||_1$ are not differentiable along the same directions at every point, $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}(\hat{\theta}\left(\boldsymbol{\lambda}\right))$ is the space spanned by the nonzero elements of $\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})$.
...
Taking the gradient of the expression above with respect to the basis of $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}(\hat{\theta}\left(\boldsymbol{\lambda}\right))$ gives us equation \eqref{eq:grad} in the context of this problem:
\begin{equation}
%\nabla \boldsymbol{\theta}||\boldsymbol{\theta}||_2 = \frac{{\boldsymbol{\theta}}}{||{\boldsymbol{\theta}}||_2}
-\frac{1}{n} \tilde{\boldsymbol{X}}_T^T(\boldsymbol{y}_T - \tilde{\boldsymbol{X}}_T \tilde{\boldsymbol\theta}(\boldsymbol \lambda))
+ \lambda_1
\begin{bmatrix}
...
+ \lambda_2 \boldsymbol{D}(\boldsymbol{z})^T\boldsymbol{D}(\boldsymbol{z}))^{-1} \boldsymbol{M}^T \tilde{\boldsymbol X}_T(\boldsymbol{\lambda}) \bigg)^{-1} 
...
-\bigg(\boldsymbol{M}^T \boldsymbol{M}
- \boldsymbol{M}^T \tilde{\boldsymbol X}_T(\boldsymbol{\lambda}) (\tilde{\boldsymbol X}_T(\boldsymbol{\lambda})^T\tilde{\boldsymbol X}_T(\boldsymbol{\lambda}))^{-1} \tilde{\boldsymbol X}_T(\boldsymbol{\lambda})^T \boldsymbol{M} + \lambda_2 \boldsymbol{D}(\boldsymbol{z})^T\boldsymbol{D}(\boldsymbol{z}) \bigg)
...
We ran two simulation studies for this paper. The purpose of this first set of simulations is to compare the performance and efficiency of grid-based descent-based joint optimization across different regularization methods.

Grid-based joint optimization was performed using cross validation over a grid of parameter values; descent-based joint optimization was performed using gradient descent with constant step size and Nesterov's gradient descent with adaptive restarts. The latter is an accelerated ``descent" method in which the step depends on the momentum from the previous iteration. The momentum grows each iteration as long as the criterion continues to decrease. Otherwise, the momentum is reset to zero. For more details, refer to the analysis by O'Donaghue and Candes.
...
Both descent-based methods were initialized at the four points $\lambda_1 = \lambda_2 = 10^i$ for $i=-2, -1, 0, 1$. Grid-based joint optimization used a $10 \times 10$ grid from $1e-6$ to $10$.
...
As shown in Table \ref{table:validation}, descent-based and grid-based joint optimization methods all had the same performance. The only major difference is in the runtimes; gradient descent was over 40\% faster than the grid-based approach and Nesterov's subgradient descent was over 50\% faster in all three regression examples.

In Figure \ref{figure:costpath}, we show plots that illustrate the variation in performance of the joint optimization -- for a single run, it is possible for gradient descent to reach values lower or higher than the grid-based approach. However, on average, we expect the methods to reach the same values. The plots also illustrate that Nesterov's gradient descent consistently requires fewer iterations.
...
We then compared tuning the two-parameter version using grid-based joint optimization to tuning the many-parameter version using gradient descent. In the results, we found that gradient descent outperformed grid-based joint optimization in both experiments.
...
% As shown later, since gradient descent for the generalized regression achieved a lower test set loss on average, we have shown that a descent-based approach for many-penalty regressions can produce better models than those from 2-penalty regressions.
...
\label{unpooledSGL}
\end{equation}
so that the number of regularization parameters is one plus the number of coefficient groups. This generalized criterion adds the flexibility of having different thresholds for setting coefficients and coefficient groups to zero. Potentially, unpooled sparse group lasso can better model covariate groups with very different distributions.

We ran three experiments with different numbers of covariate groups $m$ and total covariates $p$, as given in Table \ref{table:unpooled}. The simulation settings were similar to the previous sparse group lasso simulation study. We used the same grid for grid-based joint optimization. For gradient descent, the $m+1$ regularization parameters were initialized at three points: $1\text{\sc{e}-}4 \times \boldsymbol 1^T$, $1\text{\sc{e}-}3 \times \boldsymbol 1^T$, and $1\text{\sc{e}-}2 \times \boldsymbol 1^T$.
