...
\begin{flushleft}
{\Large
\textbf\newline{Tuning Parameter Selection based on Validation-Error Descent}
}
\newline
\\
...
\caption{Joint Optimization with Gradient Descent}
  ...
  \begin{algorithmic}
		...
		\STATE{
              	...
              	\STATE{
			Construct matrix $U^{(k)}$, an orthonormal basis of $\Omega^{L_T(\cdot, \boldsymbol{\lambda})}\left (\hat{\theta}(\boldsymbol{\lambda}^{(k)}) \right )$.
		}
              	\STATE{
			Define the locally equivalent joint optimization problem
                            \begin{equation}
                            \begin{array}{c}
                            \min_{\boldsymbol \lambda \in \Lambda} L(y_V, f_{U^{(k)} \hat{\boldsymbol \beta} (\boldsymbol \lambda) }(X_V)) \\
                            \text{where } \hat{\boldsymbol \beta} (\boldsymbol \lambda) =
                            \argmin_{\boldsymbol \beta}
...
In addition to judging models by their performances on the test set, we measured the error of the fitted linear effects and the nonparametric estimates. These correspond to the $\beta$ error ($||\beta - \hat \beta||_2^2$) and $\theta$ error ($|| g(z) - \theta ||_2^2$), respectively.
